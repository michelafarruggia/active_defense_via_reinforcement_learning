...
def evaluate_dnf_improved(formula, true_props):
    def evaluate_subformula(subformula):
        if '|' in subformula:
            return any(evaluate_subformula(f.strip()) for f in subformula.split('|'))
        if '&' in subformula:
            return all(evaluate_subformula(f.strip()) for f in subformula.split('&'))
        if subformula.startswith('!'):
            return not evaluate_subformula(subformula[1:].strip())
        if subformula == "True":
            return True
        if subformula == "False":
            return False
        return subformula in true_props

    # Gestione delle parentesi
    while '(' in formula:
        formula = re.sub(r'\(([^()]*)\)', lambda m: str(evaluate_subformula(m.group(1))), formula)
    
    return evaluate_subformula(formula)
